-- Metadata
IconAssets = group{quality=9.95}
Icon = image{"icon.png"}

Music = tracker{"sv_ttt.xm"}
CoinSound = sound{"chest.wav"}
KeySound = sound{"key.wav"}

GameAssets = group{}

Emptiness = image{"art/emptiness.png", pinned=true, width=8, height=8}
Unvisited = image{"art/unvisited.png", pinned=true, width=8, height=8}
Visited = image{"art/visited.png", pinned=true, width=8, height=8}
Transparent = image{"art/transparent.png", pinned=true, width=8, height=8}
Highlight = image{"art/highlight.png", pinned=true, width=8, height=8}

Boulder = image{"art/boulder.png", pinned=true, width=64, height=64}
FlowerRed = image{"art/flower.png", pinned=true, width=64, height=64}
FlowerBlue = image{"art/flower_blue.png", pinned=true, width=64, height=64}

Snake = image{"art/snake.png", pinned=true, width=64, height=64}
Turtle = image{"art/turtle.png", pinned=true, width=64, height=64}
Frog = image{"art/frog.png", pinned=true, width=64, height=64}
Ladybug = image{"art/ladybug.png", pinned=true, width=64, height=64}

BoulderSmall = image{"art/boulder_small.png", pinned=true, width=8, height=8}
FlowerRedSmall = image{"art/flower_small.png", pinned=true, width=8, height=8}
FlowerBlueSmall = image{"art/flower_blue_small.png", pinned=true, width=8, height=8}
SnakeSmall = image{"art/snake_small.png", pinned=true, width=8, height=8}
TurtleSmall = image{"art/turtle_small.png", pinned=true, width=8, height=8}
FrogSmall = image{"art/frog_small.png", pinned=true, width=8, height=8}
LadybugSmall = image{"art/ladybug_small.png", pinned=true, width=8, height=8}

Dirt = image{"art/dirt.png"}
GrassDark = image{"art/grass_dark.png"}
GrassLight = image{"art/grass_light.png"}
Rocks = image{"art/rocks.png"}
Sand = image{"art/sand.png"}
Water = image{"art/water.png"}
ChestOpen = image{"art/chest_open.png"}
ChestClosed = image{"art/chest_closed.png"}
Key = image{"art/key.png"}

